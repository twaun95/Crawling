...
def 매일경제(search):
    url = "http://search.auction.co.kr" #싸이트 이름
    driver = webdriver.Chrome("../chromedriver.exe")
    driver.get(url)
    time.sleep(2)

    inputNews = driver.find_element_by_id("txtKeyword") #검색칸
    inputNews.send_keys(search)
    inputNews.send_keys(Keys.ENTER)

    moreInfo = driver.find_elements_by_css_selector("tbody tr .class_more a")  #더보기
    infoJss = []
...
def mkUrlList(driver):
    ...
    for pageNo in range(1,6): #2페이지까지
        url = url2 + "&pageNum={pageNo}".format(pageNo=pageNo) #페이지
        driver.get(url)
